...
//        produtos.removeAll(findProdutoPorCategoria(categoria));
    }
    ...
    public List<String> findNomesRepetidos() {
        ...
        for (int i = 0; i < produtos.size(); i++) {
            for (int j = 0; j < produtos.size(); j++) {
                if (i != j) {
                    if (produtos.get(i).getNome()
                            .equalsIgnoreCase(produtos.get(j).getNome())) {
                        if (!(repetidos.contains(produtos.get(i).getNome()))) {
                            repetidos.add(produtos.get(i).getNome());
                        }
                    }
                }
            }
