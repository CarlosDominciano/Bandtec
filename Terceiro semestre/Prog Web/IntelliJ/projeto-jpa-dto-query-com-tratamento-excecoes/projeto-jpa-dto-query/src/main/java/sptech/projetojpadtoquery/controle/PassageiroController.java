...
public class PassageiroController {
    ...
    public ResponseEntity<List<PassageiroSimplesResponse>> getSimples() {
        List<PassageiroSimplesResponse> passageiroSimples = repository.getPassageirosSimples();
        if (passageiroSimples.isEmpty())
            return ResponseEntity.status(204).build();
        return ResponseEntity.status(200).body(passageiroSimples);
